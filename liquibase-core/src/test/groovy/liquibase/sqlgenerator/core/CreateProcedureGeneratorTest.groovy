...
class CreateProcedureGeneratorTest extends Specification {
    ...
    def "Verify CreateProcedureGenerator logic correctly generates create procedure sql statement based on given procedureText and replaceIfExists attributes for MSSQL"() {
        ...
        "CREATE PROCEDURE [dbo].[procedure_1]"                                                                                                | "dbo"      | "procedure_1"       | true            | ["if object_id('[dbo].[procedure_1]', 'p') is null exec ('create procedure [dbo].[procedure_1] as select 1 a')", "CREATE OR ALTER PROCEDURE [dbo].[procedure_1]", DEFAULT_SET_ANSI_STATEMENT, DEFAULT_SET_QUOTED_IDENTIFIER_STATEMENT]
