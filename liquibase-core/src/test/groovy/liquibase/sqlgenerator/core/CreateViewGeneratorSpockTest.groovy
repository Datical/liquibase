...
class CreateViewGeneratorSpockTest extends Specification {
    ...
    def "Verify CreateViewGenerator logic correctly generates create view sql statement based on given procedureText and replaceIfExists attributes for MSSQL"() {
        ...
        "CREATE VIEW [dbo].[view_1] AS SELECT CREATE FROM EMPLOYEE"                     | "dbo"      | "view_1" | true            | true             | ["IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[DBO].[view_1]'))\n    EXEC sp_executesql N'CREATE VIEW [DBO].[view_1] AS SELECT ''This is a code stub which will be replaced by an Alter Statement'' as [code_stub]'", "CREATE OR ALTER VIEW [dbo].[view_1] AS SELECT CREATE FROM EMPLOYEE", DEFAULT_SET_ANSI_STATEMENT, DEFAULT_SET_QUOTED_IDENTIFIER_STATEMENT]
        ...
        "SELECT LastName FROM [dbo].view_1;"                                            | "dbo"      | "view_1" | true            | false            | ["IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[DBO].[view_1]'))\n    EXEC sp_executesql N'CREATE VIEW [DBO].[view_1] AS SELECT ''This is a code stub which will be replaced by an Alter Statement'' as [code_stub]'", "CREATE OR ALTER VIEW [dbo].[view_1] AS SELECT LastName FROM [dbo].view_1;", DEFAULT_SET_ANSI_STATEMENT, DEFAULT_SET_QUOTED_IDENTIFIER_STATEMENT]
    }
