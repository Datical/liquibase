...
public class CreateTableGeneratorTest extends AbstractSqlGeneratorTest<CreateTableStatement> {
    ...
    public CreateTableGeneratorTest() throws Exception {
        setUnderTest(new CreateTableGenerator());
    }
        ...
        for (Database database : TestContext.getInstance().getAllDatabases()) {
            if (database instanceof OracleDatabase) {
                ...
                if (shouldBeImplementation(database)) {
                    assertEquals("CREATE TABLE CATALOG_NAME.TABLE_NAME (COLUMN1_NAME TIMESTAMP DEFAULT null)", this.getGeneratorUnderTest().generateSql(statement, database, null)[0].toSql());
                }
            }
        }
    }
    ...
    public void testAutoIncrementMSSQLDatabase() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof MSSQLDatabase) {
    			...
    			assertEquals("CREATE TABLE [CATALOG_NAME].[SCHEMA_NAME].[TABLE_NAME] ([COLUMN1_NAME] [bigint] IDENTITY (1, 1) NOT NULL)", generatedSql[0].toSql(), "Error on " + database);
    		}
    	}
    }
    ...
    public void testAutoIncrementSybaseDatabase() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof SybaseDatabase) {
    			...
    			assertEquals("CREATE TABLE [SCHEMA_NAME].[TABLE_NAME] ([COLUMN1_NAME] BIGINT IDENTITY NULL)", generatedSql[0].toSql(), "Error with " + database);
    		}
    	}
    }
