...
public class ColumnSnapshotGenerator extends JdbcSnapshotGenerator {
    private static final Logger LOGGER = LogFactory.getInstance().getLog(ColumnSnapshotGenerator.class.getSimpleName());
  /**
  ...
  private static final String LIQUIBASE_COMPLETE = "liquibase-complete";
    ...
    private static final Map<String, String> POSTGRES_SERIAL_TYPES = Map.of(
            "smallserial", "int2",
            "serial", "int4",
            "bigserial", "int8");
    ...
    protected DatabaseObject snapshotObject(DatabaseObject example, DatabaseSnapshot snapshot) throws DatabaseException {
        ...
        try {
            ...
            if (!metaDataColumns.isEmpty()) {
              ...
              column = readColumn(data, relation, database, schema);
        ...
        if (foundObject instanceof Relation) {
            ...
            try {
                ...
                for (CachedRow row : allColumnsMetadataRs) {
                    Column column = readColumn(row, relation, database, schema);
                    setAutoIncrementDetails(column, database, snapshot);
                    populateValidateNullableIfNeeded(column, metaDataNotNullConst, database);
    ...
    protected Column readColumn(CachedRow columnMetadataResultSet, Relation table, Database database, Schema schema) throws SQLException, DatabaseException {
        ...
        if (database instanceof PostgresDatabase
                ...
                &&  POSTGRES_SERIAL_TYPES.entrySet().stream().anyMatch(entry -> entry.getKey().equalsIgnoreCase(type.getTypeName()))
                && isSequenceUsedForPkNonAutoGenerated(database, schema, (DatabaseFunction) defaultValue)) {
            column.setAutoIncrementInformation(null);
            type.setTypeName(POSTGRES_SERIAL_TYPES.get(type.getTypeName()));
        }
    ...
    private boolean isSequenceUsedForPkNonAutoGenerated(Database database, Schema schema, DatabaseFunction defaultValue) {
        String defaultValueText = defaultValue.getValue();
        String[] defaultValuePieces = defaultValueText.split("'");
        if (defaultValuePieces.length == 3 && "nextval(".equalsIgnoreCase(defaultValuePieces[0])) {
            String sequenceName = defaultValuePieces[1];
            String sql = getPostgresQueryToCheckIfSequenceIsNonAutoGenerated(database, schema, sequenceName);
            try {
                List<Map<String, ?>> sequenceQueryResult = ExecutorService.getInstance().getExecutor(database).queryForList(new RawSqlStatement(sql));
                return !sequenceQueryResult.isEmpty();
            } catch (DatabaseException e) {
                LOGGER.warning("Error is thrown during checking if Sequence is auto generated", e);
            }
        }
        return false;
    }

    private String getPostgresQueryToCheckIfSequenceIsNonAutoGenerated(Database database, Schema schema, String sequenceName) {
        int version = 9;
        try {
            version = database.getDatabaseMajorVersion();
        } catch (DatabaseException e) {
            LOGGER.warning("Failed to retrieve database version during getting SQL query to check if sequence is auto generated", e);
        }
        if (version < 10) {
